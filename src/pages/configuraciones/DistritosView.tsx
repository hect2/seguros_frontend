...
export function DistritosView() {
  ...
  const [distritoToDelete, setDistritoToDelete] = useState<Distrito | null>(null);
  const navigate = useNavigate();
  const filteredDistritos = mockDistritos.filter(distrito => {
    const matchesSearch = distrito.nombre.toLowerCase().includes(searchTerm.toLowerCase()) || distrito.codigo.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesEstado = estadoFilter === 'all' || distrito.estado === estadoFilter;
  ...
  const handleDelete = (distrito: Distrito) => {
    setDistritoToDelete(distrito);
    setIsDeleteDialogOpen(true);
  };
  const handleViewDistrict = (distrito: Distrito) => {
    navigate(`/configuraciones/asignacion-territorial/distritos/${distrito.id}`);
  };
  const confirmDelete = () => {
    alert(`Eliminando distrito: ${distritoToDelete?.nombre}`);
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    {filteredDistritos.map((distrito, index) => <tr key={distrito.id} className={`border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`} onClick={() => handleViewDistrict(distrito)}>
                        ...
                        <td className="py-4 px-6">
                          <div>
                            ...
                            {distrito.descripcion && <div className="text-sm text-gray-500">
                        ...
                        <td className="py-4 px-6 text-center">
                          <button onClick={e => {
                        e.stopPropagation();
                        navigate(`/configuraciones/asignacion-territorial/oficinas?distrito=${distrito.codigo}`);
                        ...
                        <td className="py-4 px-6 text-center">
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${distrito.estado === 'Activo' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                            {distrito.estado}
                          </span>
                        </td>
              ...
              <div className="border-t border-gray-200 px-6 py-4 flex items-center justify-between">
                <div className="text-sm text-gray-600">
                  Mostrando 1 a 6 de {filteredDistritos.length} distritos
                </div>
