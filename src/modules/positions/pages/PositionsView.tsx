...
export function PositionsView() {
  ...
  const { data, createPosition, updatePosition } = usePositions();
  ...
  const confirmDelete = () => {
    alert(`Eliminando cargo: ${positionToDelete?.name}`);
    setIsDeleteDialogOpen(false);
    setPositionToDelete(null);
  };

  const handleSubmit = async (positionLike: Partial<Position>) => {
    const processedData = {
      ...positionLike,
    };

    if (modalMode === 'create') {
      await createPosition(processedData, {
        onSuccess: () => {
        ...
        onError: () => {
          toast.error('No se pudo crear el cargo', {
            position: 'top-right',
          });
        }
      });
    } else {
      await updatePosition({ ...processedData, id: selectedPosition!.id }, {
        onSuccess: () => {
          setIsModalOpen(false);
  ...
  return (
    <div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {filteredPositions.map((position, index) => <tr key={position.id} className={`border-b border-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                        <td className="py-4 px-6">{position.name}</td>
                        <td className="py-4 px-6 text-gray-600">{position.description}</td>
                        <td className="py-4 px-6 text-center text-gray-600 text-sm">
                          {new Date(position.updated_at).toLocaleDateString()}
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center justify-center space-x-2">
                            <button onClick={() => handleEdit(position)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Editar">
                              <Pencil size={18} />
                            </button>
                ...
                <CustomPagination totalPages={data?.last_page || 1} from={data?.from || 1} to={data?.to || 1} totalItems={data?.total || 1} module='cargos' />
              </div>
            </main>
          </PermissionGuard>
        </div>
      ...
      <ConfirmDialog
        ...
        message={`¿Estás seguro de que deseas eliminar el cargo "${positionToDelete?.name}"? Esta acción no se puede deshacer.`}
