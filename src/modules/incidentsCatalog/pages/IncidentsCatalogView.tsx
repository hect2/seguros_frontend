...
import { useIncidentsCatalog } from '../hooks/useIncidentsCatalog';
import { IncidentCatalogModal } from '../components/IncidentCatalogModal';
import { IncidentCatalog } from '../interfaces/incidents.catalog.interface';

export function IncidentsCatalogView() {
  const { user } = useAuthStore();

  const { data, createIncidentCatalog, updateIncidentCatalog } = useIncidentsCatalog();
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | 'Activo' | 'Inactivo'>('all');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [modalMode, setModalMode] = useState<'create' | 'edit'>('create');
  const [selectedPosition, setSelectedPosition] = useState<IncidentCatalog | null>(null);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [positionToDelete, setPositionToDelete] = useState<IncidentCatalog | null>(null);
  const [searchParams, setSearchParams] = useSearchParams();
  const [searchValue, setSearchValue] = useState(
    searchParams.get("search") || ""
  );

  const incidentsCatalog = data?.data || [];
  const filteredIncidents = incidentsCatalog.filter(incident => {
  ...
  const handleEdit = (incident: IncidentCatalog) => {
    setModalMode('edit');
    setSelectedPosition(incident);
    setIsModalOpen(true);
  };

  const handleDelete = (incident: IncidentCatalog) => {
  ...
  const handleSubmit = async (positionLike: Partial<IncidentCatalog>) => {
    const processedData = {
      ...positionLike,
    };

    if (modalMode === 'create') {
      await createIncidentCatalog(processedData, {
        onSuccess: () => {
      ...
      await updateIncidentCatalog({ ...processedData, id: selectedPosition!.id }, {
        onSuccess: () => {
          setIsModalOpen(false);
  ...
  return (
    <div>
            ...
            <main className="p-4 lg:p-8">
              <div className="mb-6">
                <h1 className="text-3xl font-bold text-gray-800 mb-2">
                  Gestión de Catalogo de Novedades
                </h1>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {filteredIncidents.map((incident, index) => <tr key={incident.id} className={`border-b border-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                        <td className="py-4 px-6">{incident.name}</td>
                        {/* <td className="py-4 px-6 text-gray-600">{incident.type}</td> */}
                        <td className="py-4 px-6 text-center text-gray-600 text-sm">
                          {new Date(incident.updated_at).toLocaleDateString()}
                        </td>
                        <td className="py-4 px-6">
                          <div className="flex items-center justify-center space-x-2">
                            <button onClick={() => handleEdit(incident)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Editar">
                              <Pencil size={18} />
                            </button>
      ...
      <IncidentCatalogModal
        onClose={() => setIsModalOpen(false)}
        mode={modalMode}
        isOpen={isModalOpen}
        incidentCatalog={selectedPosition}
      ...
      <ConfirmDialog
        ...
        message={`¿Estás seguro de que deseas eliminar la novedad "${positionToDelete?.name}"? Esta acción no se puede deshacer.`}
