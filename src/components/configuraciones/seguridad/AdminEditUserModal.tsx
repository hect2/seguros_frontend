...
export function AdminEditUserModal({
  ...
  const [selectedDistritos, setSelectedDistritos] = useState<string[]>(user.distrito ? [user.distrito] : []);
  ...
  const handleSave = () => {
    onSave({
      ...user,
      ...formData,
      distrito: selectedDistritos[0] || user.distrito
    });
  };
  useEffect(() => {
  ...
  return <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
          ...
          {activeTab === 'seguridad' && <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleInputChange} placeholder="Dejar vacío para mantener la actual" className="w-full px-4 py-2.5 pr-24 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#cf2e2e] focus:border-transparent" />
              ...
              {formData.password && <div className="flex items-start space-x-2">
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-between">
          ...
          <button onClick={handleSave} className="px-8 py-2.5 bg-[#cf2e2e] text-white rounded-lg hover:bg-[#b52626] transition-colors font-medium">
            Guardar Cambios
          </button>
        </div>
      </div>
