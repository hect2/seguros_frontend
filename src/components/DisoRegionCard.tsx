...
export function DisoRegionCard() {
  const [activeIndex, setActiveIndex] = useState<number | null>(null);
  const data = [{
    name: 'OTR Quetzaltenango',
    shortName: 'Quetzaltenango',
    value: 101,
    color: '#3b82f6'
  }, {
    name: 'OTR Huehuetenango',
    shortName: 'Huehuetenango',
    value: 167,
    color: '#f97316'
  }, {
    name: 'OTR San Marcos',
  ...
  return <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data}>
          <XAxis dataKey="shortName" angle={-45} textAnchor="end" height={80} tick={{
          fontSize: 11
        }} />
          <YAxis />
          <Tooltip content={({
          ...
          <Bar dataKey="value" radius={[8, 8, 0, 0]} onMouseEnter={(_, index) => setActiveIndex(index)} onMouseLeave={() => setActiveIndex(null)}>
